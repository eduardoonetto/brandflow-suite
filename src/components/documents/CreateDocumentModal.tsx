 ...
 import { DocumentSigner, DocumentTemplate, TemplateType } from '@/types';
 ...
 export function CreateDocumentModal({ open, onOpenChange, mode = 'document' }: CreateDocumentModalProps) {
   ...
   const canProceedToSigners = mode === 'document' && (
     (documentType === 'template' && selectedTemplateId) ||
     (documentType === 'upload' && pdfFile)
   );
 
   const canCreate = mode === 'template' 
               ...
               <div className="space-y-4">
                 ...
                 {mode === 'document' && (
                   <div>
                     ...
                     <Input
                       placeholder="Dejar vacío para usar título de plantilla"
                       value={title}
                       onChange={(e) => setTitle(e.target.value)}
                     />
                   </div>
